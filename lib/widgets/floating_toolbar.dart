...
class _FloatingToolbarState extends State<FloatingToolbar> {
  ...
  void _showPopup(BuildContext ctx, GlobalKey key, Widget popup, double width) {
    ...
    final arrowLeft = (offset.dx + box.size.width / 2 - left - 7).clamp(12.0, width - 24.0);
    final origX = (offset.dx + box.size.width / 2 - left).clamp(20.0, width - 20.0);
          ...
          child: Row(
            children: [
              if (!aiMode)
                ...
                Expanded(child: _AITrack(controller: aiController, loading: aiLoading)),
              _ConfirmBtn(aiMode: aiMode, aiLoading: aiLoading, aiController: aiController),
            ],
          ),
        ),
      ),
        ...
        SingleChildScrollView(
          ...
          child: Row(
            children: [
              ...
              _AlignBtn(icon: Icons.format_align_center,  align: 'center',  current: st.align),
              _AlignBtn(icon: Icons.format_align_right,   align: 'right',   current: st.align),
...
class _ConfirmBtn extends StatefulWidget {
  ...
  final TextEditingController aiController;
  const _ConfirmBtn({required this.aiMode, required this.aiLoading, required this.aiController});
  @override
        ...
        child: AnimatedScale(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: st.aiMode && !widget.aiLoading ? Colors.transparent : kBorder,
                width: 1.5,
              ),
            ),
            child: widget.aiLoading
                ? const _AIDots()
                : Icon(
                    st.aiMode ? Icons.send : Icons.check,
                    size: 16,
      ...
      children: List.generate(3, (i) => AnimatedBuilder(
        ...
        builder: (_, __) => Container(
          ...
          transform: Matrix4.diagonal3Values(
              _anims[i].value, _anims[i].value, 1),
...
class _AlignBtn extends StatelessWidget {
  ...
  const _AlignBtn({required this.icon, required this.align, required this.current});
  @override
  Widget build(BuildContext context) {
