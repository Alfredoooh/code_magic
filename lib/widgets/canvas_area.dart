...
int _charsPerLine(double fs) =>
    (_kUsableWidth / (fs * 0.52)).floor().clamp(1, 9999);

int _linesPerPage(double fs) =>
    (_kUsableHeight / (fs * 1.85)).floor().clamp(1, 9999);
...
// ── Config partilhada do QuillEditor ─────────────────────
quill_lib.QuillEditorConfigurations _editorConfig(
  ...
  FocusNode? focus,
    ...
    quill_lib.QuillEditorConfigurations(
      ...
      // Menu de seleção nativo substituído pelo nosso
      contextMenuBuilder: ctrl == null
          ? null
          : (context, rawEditor) => _SelectionMenu(
                controller: ctrl,
                rawEditor: rawEditor,
              ),
    );
...
class _SelectionMenu extends StatelessWidget {
  ...
  final dynamic rawEditor; // RawEditorState

  const _SelectionMenu({required this.controller, required this.rawEditor});

  void _fmt(quill_lib.Attribute attr) =>
    ...
    return AdaptiveTextSelectionToolbar.buttonItems(
      ...
      buttonItems: [
        ContextMenuButtonItem(
          label: 'Negrito',
          onPressed: () => _fmt(quill_lib.Attribute.bold),
        ),
        ContextMenuButtonItem(
          label: 'Itálico',
          onPressed: () => _fmt(quill_lib.Attribute.italic),
        ),
        ContextMenuButtonItem(
          label: 'Sublinhado',
          onPressed: () => _fmt(quill_lib.Attribute.underline),
        ),
        ContextMenuButtonItem(
          label: 'Riscado',
          onPressed: () => _fmt(quill_lib.Attribute.strikeThrough),
        ),
        ContextMenuButtonItem(
          label: 'Apagar',
          onPressed: _delete,
...
// ── Decoração partilhada da página ────────────────────────
...
class _CanvasAreaState extends State<CanvasArea> {
  ...
  void _computeScale(double availWidth) {
    final base = availWidth < kPageWidth ? availWidth / kPageWidth : 1.0;
      ...
      return GestureDetector(
        ...
        child: Container(
          ...
          child: SingleChildScrollView(
            ...
            // ── Centra horizontalmente dentro do scroll ──
            child: Center(
...
class _ScrollPageState extends State<_ScrollPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () {}, // absorve — não propaga ao fundo
        ...
        child: Padding(
          ...
          child: Focus(
            ...
            child: quill_lib.QuillEditor.basic(
              ...
              configurations: _editorConfig(fs,
                  ctrl: st.quill, focus: st.focusNode),
            ),
          ),
        ),
      ),
...
class _A4PageState extends State<_A4Page> {
  ...
  quill_lib.QuillController _pageCtrl(AppEditorState st) {
    ...
    final end = (charOffset + lpp * cpl).clamp(charOffset, math.max(0, text.length - 1));
    final snippet = charOffset < text.length
        ? text.substring(charOffset, end as int)
        : '';
