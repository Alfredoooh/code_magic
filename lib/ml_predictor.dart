...
class MLPredictor {
  ...
  // Timer reference (optionally cancellable)
  Timer? _onlineTimer;
  ...
  void _initializeAdvancedModel() {
    ...
    _weightsLayer1 = List.generate(
        _inputSize, (_) => List.generate(_hiddenSize1, (_) => random.nextDouble() * 2 - 1));
    _biasLayer1 = List.generate(_hiddenSize1, (_) => random.nextDouble() * 0.1);

    // Inicializar camada 2 (hidden1 -> hidden2)
    _weightsLayer2 = List.generate(
        _hiddenSize1, (_) => List.generate(_hiddenSize2, (_) => random.nextDouble() * 2 - 1));
  ...
  void _startOnlineDataFetching() {
    ...
    _onlineTimer = Timer.periodic(const Duration(minutes: 5), (_) {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data != null && data['data'] != null && data['data'].isNotEmpty) {
          _marketSentiment = {
            'fear_greed_index': double.tryParse(data['data'][0]['value'].toString()) ?? 50.0,
  ...
  void addTradeResult(bool won, double profit) {
    ...
    accuracy = totalPredictions > 0 ? correctPredictions / totalPredictions : 0.0;

    // Treinar modelo com backpropagation simplificado
    _trainDeepModel(won);
  ...
  void _analyzePrediction() {
    ...
    final finalPrediction = (deepPrediction * 0.4 + lstmPrediction * 0.3 + ensemblePrediction * 0.3);
  ...
  double _predictDeepNetwork(List<double> features) {
    ...
    for (int j = 0; j < _hiddenSize2; j++) {
      ...
      for (int i = 0; i < _hiddenSize1; i++) {
        // proteção caso layer1Output seja menor (não deve, mas por segurança)
        final val = (i < layer1Output.length) ? layer1Output[i] : 0.0;
        sum += val * _weightsLayer2[i][j];
      }
    ...
    for (int i = 0; i < _hiddenSize2; i++) {
      final val = (i < layer2Output.length) ? layer2Output[i] : 0.0;
      output += val * _weightsOutput[i];
    }
  ...
  double _predictEnsemble(List<double> features) {
    ...
    weightedSum += trendFollower * (_ensembleModels[0]['weight'] as double);
    weightedSum += meanReversion * (_ensembleModels[1]['weight'] as double);
    weightedSum += momentum * (_ensembleModels[2]['weight'] as double);
    weightedSum += patternRecog * (_ensembleModels[3]['weight'] as double);
  ...
  double _predictTrendFollower(List<double> features) {
    ...
    final trend = features.length > 5 ? features[5] : 0.0;
    final trendStrength = features.length > 6 ? features[6] : 0.0;
    final sma20 = features.length > 2 ? features[2] : 0.0;
    final ema12 = features.length > 3 ? features[3] : 0.0;
  ...
  double _predictMeanReversion(List<double> features) {
    ...
    final rsi = features.length > 9 ? features[9] : 0.5;
    final bollingerBands = features.length > 16 ? features[16] : 0.5;
    final support = features.length > 20 ? features[20] : 0.0;
    final resistance = features.length > 21 ? features[21] : 0.0;
  ...
  double _predictMomentum(List<double> features) {
    ...
    final roc = features.length > 22 ? features[22] : 0.0;
    final momentum = features.length > 23 ? features[23] : 0.5;
    final macd = features.length > 13 ? features[13] : 0.0;
  ...
  double _predictPatternRecognition(List<double> features) {
    ...
    final candlePattern = features.length > 17 ? features[17] : 0.0;
    final priceAction = features.length > 18 ? features[18] : 0.0;
  ...
  void _trainDeepModel(bool won) {
    ...
    // Backpropagation simplificado com ajuste básico
    final learningFactor = _learningRate * (1 + accuracy);

    // Atualizar pesos da camada de saída (aproximação)
    for (int i = 0; i < _weightsOutput.length; i++) {
      final delta = (i < _hiddenState.length) ? _hiddenState[i] : 0.0;
      _weightsOutput[i] += learningFactor * error * delta;
    }
  ...
  void _updateEnsembleWeights(bool won) {
    ...
    for (var model in _ensembleModels) {
      if (won) {
        model['accuracy'] = (model['accuracy'] as double) * 0.95 + 0.05;
      } else {
        model['accuracy'] = (model['accuracy'] as double) * 0.95;
      }
    }
  ...
  void _updatePatternSuccessRate(bool won) {
    ...
    final patterns = (currentPrediction!['patterns'] as List).cast<String>();
  ...
  double _calculateAdvancedConfidence(
    ...
    final volatility = features.length > 14 ? features[14] : 0.5;
    final marketScore =
        (1.0 - volatility) * 0.5 + ((_marketSentiment['fear_greed_index'] ?? 50.0) / 100.0) * 0.5;
    ...
    if (patterns.isNotEmpty) {
      final sumPattern = patterns.map((p) => (_patternSuccessRate[p] ?? 0) / 100.0).fold(0.0, (a, b) => a + b);
      patternConfidence = (sumPattern / patterns.length).clamp(0.0, 1.0);
    }
    confidence += patternConfidence * 0.1;
  ...
  double _calculateRSI(List<double> prices) {
    ...
    for (int i = prices.length - 13; i < prices.length; i++) {
  ...
  double _calculateATR(List<double> prices) {
    ...
    final tr = trueRanges.sublist(max(0, trueRanges.length - 14));
    return tr.reduce((a, b) => a + b) / 14 / prices.last;
  }
  ...
  bool _detectWedge(List<double> prices) {
    ...
    return (highRange - lowRange).abs() / prices.last < 0.03;
  }
